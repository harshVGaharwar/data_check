...
class _ReportPageState extends State<ReportPage> {
          ...
          child: Center(
            child: Column(
              children: [
                ...
                SizedBox(height: 12),
                Text(
                  'No records found for the selected template.',
  ...
  Set<String> _matchedColumns(String query) {
    ...
    for (final item in _results) {
      ...
      if (_resolveStatus(
        item['status']?.toString() ?? item['isApproved']?.toString() ?? '',
      ).toLowerCase().contains(q)) {
        matched.add('Status');
      }
    }
  ...
  TableRow _buildTableRow(Map<String, dynamic> item, int index) {
    ...
    final statusRaw =
        item['status']?.toString() ?? item['isApproved']?.toString() ?? '';
    final status = _resolveStatus(statusRaw);
...
// ── Searchable overlay (shared pattern) ───────────────────────────────────────
