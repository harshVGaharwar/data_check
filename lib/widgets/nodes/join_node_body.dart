...
class JoinNodeBody extends StatelessWidget {
  ...
  void _submitMapping(BuildContext context, PipelineController ctrl, PipelineNode node) async {
    ...
    // ── 1. Source Summary ──
    final sources = ctrl.nodes.where((n) => n.type.isSource).map((s) => {
      'sourceId': s.id,
      'sourceName': s.name,
    ...
    // ── 2. Join Mappings ──
    final joinNodes = ctrl.nodes.where((n) => n.type == NodeType.join).map((j) {
      final jMappings = j.mappings.where((m) => m.isValid).toList();
      final connSrcIds = ctrl.edges.where((e) => e.toNodeId == j.id).map((e) => e.fromNodeId).toList();
      return {
        'joinNodeId': j.id,
        'mappings': jMappings.map((m) {
          final lSrc = ctrl.findNode(m.leftSourceId);
          final rSrc = ctrl.findNode(m.rightSourceId);
          return {
        ...
        'connectedSources': connSrcIds,
      };
    }).toList();

    // ── 3. Edges (graph structure) ──
    final edges = ctrl.edges.map((e) => {
      'from': e.fromNodeId,
      'to': e.toNodeId,
    }).toList();
